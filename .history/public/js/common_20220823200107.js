...
const ratingScoreP = document.getElementsByClassName("rating-score-p");
  for (let k=0; k < ratingScoreValue.length; k++) {
    if (document.getElementsByClassName('star')) {
      ...
      for (let i = 0; i < stars.length; i++) {
        ...
        const starClick = (e) => {
          ...
          ratingScoreValue[i].value = [1];
          ratingScoreP[i].textContent = [1];
          stars[i].textContent = '★';
          for (let j = 0; j < i; j++) {
            stars[j].textContent = '★';
            ratingScoreValue[k].value = [i + 1];
            ratingScoreP[k].textContent = [i + 1];
          }
        }
