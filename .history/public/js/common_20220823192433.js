...
if (document.getElementsByClassName('star')) {
  ...
  for (let i = 0; i < stars.length; i++) {
    ...
    const starClick = (e) => {
      ...
      const ratingScoreValue = document.getElementsByClassName("rating-score-value");
      const ratingScoreValue = document.getElementsByClassName("rating-score-p").textContent = [1];

      document.getElementsByClassName("rating-score-value").value = [1];
      document.getElementsByClassName("rating-score-p").textContent = [1];
      stars[i].textContent = '★';
      for (let j = 0; j < i; j++) {
        stars[j].textContent = '★';
        document.getElementsByClassName("rating-score-value").value = [i + 1];
        document.getElementsByClassName("rating-score-p").textContent = [i + 1];
      }
    }
