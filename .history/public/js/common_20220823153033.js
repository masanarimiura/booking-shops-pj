...
// my-page ratingモーダルの on off
if (document.getElementsByClassName('modal-rating')) {
  for (let i = 0; i < document.getElementsByClassName('modal-rating').length; ++i) {
    const openBtn = document.getElementsByClassName('openBtn-rating')[i];
    const closeBtn = document.getElementsByClassName('closeBtn-rating')[i];
    const modal = document.getElementsByClassName('modal-rating')[i];
    openBtn.addEventListener('click', () => {
      modal.style.display = 'block';
    })
    closeBtn.addEventListener('click', () => {
...
if (document.getElementsByClassName('star')) {
  ...
  for (let i = 0; i < stars.length; i++) {
    ...
    const starMouseover = (e) => {
      stars[i].textContent = '★';
      document.getElementById("rating-score-p").textContent = [1];
      for (let j = 0; j < i; j++) {
        stars[j].textContent = '★';
        document.getElementById("rating-score-p").textContent = [i + 1];
      }
    }
    stars[i].addEventListener('mouseover', starMouseover);
    // 星マークからマウスが離れた時のイベント
    const starMouseout = (e) => {
      document.getElementById("rating-score-p").textContent = '…';
    ...
    // 星マークをクリックした時のイベント
    const starClick = (e) => {
        stars[0].removeEventListener('mouseover', starMouseover);
        stars[0].removeEventListener('mouseout', starMouseout);
        stars[1].removeEventListener('mouseover', starMouseover);
        stars[1].removeEventListener('mouseout', starMouseout);
        stars[2].removeEventListener('mouseover', starMouseover);
        stars[2].removeEventListener('mouseout', starMouseout);
        stars[3].removeEventListener('mouseover', starMouseover);
        stars[3].removeEventListener('mouseout', starMouseout);
        stars[4].removeEventListener('mouseover', starMouseover);
        stars[4].removeEventListener('mouseout', starMouseout);
        stars[5].removeEventListener('mouseover', starMouseover);
        stars[5].removeEventListener('mouseout', starMouseout);
      for (let j=0; j < stars.length; j++) {
        stars[j].textContent = '☆';
      }
      stars[i].textContent = '★';
      document.getElementById("rating-score-value").value = [1];
      document.getElementById("rating-score-p").textContent = [1];
      for (let j = 0; j < i; j++) {
        stars[j].textContent = '★';
        document.getElementById("rating-score-value").value = [i + 1];
        document.getElementById("rating-score-p").textContent = [i + 1];
      }
