    ...
    <div class="modal-delete-error">
      <div class="modal__content">
        <div class="modal__content-inner">
          ...
          <p>※ご予約の変更ができませんでした。ご予約の変更を希望される場合は、{{ $error }}</p>
          @endforeach
          <div class="modal__booked-data">
            <table>
              <tr>
          ...
          <form action="{{ route('destroyBooking') }}" method="post">
            ...
            <div>
              <input type="text" name="booking_id" value="{{$booking->id}}" style="display:none">
              <input type="submit" value="予約をキャンセルする" class="cancel-btn">
            </div>
          </form>
          <a href="{{ route('my_page') }}"><input type="button" value="戻る" class="return-btn closeBtn-delete-error">
        </div>
      </div>
    </div>
    @endif
